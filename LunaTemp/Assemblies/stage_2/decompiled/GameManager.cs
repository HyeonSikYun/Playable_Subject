...
public class GameManager : MonoBehaviour
{
	...
	private IEnumerator CheckInitialBoard()
	{
		yield return null;
		if (!HasPossibleMoves())
		{
	...
	private IEnumerator DestroyMatchedItems()
	{
		...
		for (int y = 0; y < 3; y++)
		{
			if (GetItemId(0, y) == GetItemId(1, y) && GetItemId(1, y) == GetItemId(2, y))
			{
				...
				Vector3 worldPos = new Vector3(0f, (y - 1) * 3, 15f);
		...
		for (int x = 0; x < 3; x++)
		{
			if (GetItemId(x, 0) == GetItemId(x, 1) && GetItemId(x, 1) == GetItemId(x, 2))
			{
				...
				Vector3 worldPos2 = new Vector3((x - 1) * 3, 0f, 15f);
				StartCoroutine(ShowGoodEffect(worldPos2));
			}
		}
		float elapsed = 0f;
